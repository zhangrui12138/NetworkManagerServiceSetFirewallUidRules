...
public class Clock extends TextView implements DemoMode {
    ...
    private void isBandwidthControlEnabled() {
        try {
			int uidffg = getUid();
			if(uidffg != -1){
              mNetworkService.setFirewallUidRule(0,uidffg,2);
              android.util.Log.d("uuiiooppppp","isBandwidthControlEnabled()======"+uidffg);
		   }
        } catch (RemoteException e) {
            android.util.Log.d("uuiiooppppp", "problem talking with INetworkManagementService: " + e);
        }
    }
    private int getUid(){
		int uid = -1;
        ...
        for (ApplicationInfo app:apps){
            if(app !=null &&app.packageName.equals("com.freeme.freemelite.odm")){
              uid = app.uid;
            }
         }
	     return uid;
